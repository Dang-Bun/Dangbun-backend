...
//
//import static com.dangbun.domain.cleaning.refactor.adapter.out.CleaningRepeatType.WEEKLY;
...
//        }
//
//        if (cleaningRepository.existsByNameAndDuty_DutyIdAndPlace(request.cleaningName(), duty.getDutyId().value(), place)) {
//            throw new CleaningAlreadyExistsException(CLEANING_ALREADY_EXISTS);
//        }
//
//        String repeatDaysStr = (request.repeatType() == CleaningRepeatType.WEEKLY && request.repeatDays() != null)
...
//        );
//
//        CleaningJpaEntity savedCleaningJpaEntity = cleaningRepository.save(cleaningJpaEntity);
//
...
//                    .map(m -> MemberCleaning.builder().member(m).cleaning(savedCleaningJpaEntity).build())
//                    .toList();
...
//                .toList();
//
//        List<CleaningDateJpaEntity> cleaningDates = parsedDates.stream()
//                .map(date -> CleaningDateJpaEntity.builder()
//                        .date(date)
//                        .cleaning(savedCleaningJpaEntity)
//                        .build())
//                .toList();
//        createChecklistService.createChecklistByDateAndTime(savedCleaningJpaEntity, cleaningDates, place);
//        cleaningDateRepository.saveAll(cleaningDates);
//
//        return PostCleaningResponse.of(savedCleaningJpaEntity.getCleaningId());
//    }
//
//
...
//        Place place = MemberContext.get().getPlace();
//
//        CleaningJpaEntity cleaningJpaEntity = cleaningRepository.findWithDutyNullableById(cleaningId)
...
//        );
//
//        cleaningRepository.save(cleaningJpaEntity);
//
...
//                .map(m -> MemberCleaning.builder().member(m).cleaning(cleaningJpaEntity).build())
//                .toList();
...
//                .orElseThrow(() -> new CleaningNotFoundException(CLEANING_NOT_FOUND));
//
//        List<Checklist> checklists = checklistRepository.findByCleaning_CleaningId(cleaningJpaEntity.getCleaningId());
